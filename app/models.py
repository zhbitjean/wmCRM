...
class ProjectCompany(Base, AuditMixin):
    __tablename__="project_companies"; __table_args__=(UniqueConstraint("project_id","company_id","project_role"),); id: Mapped[int]=mapped_column(primary_key=True); project_id: Mapped[int]=mapped_column(ForeignKey("projects.id")); company_id: Mapped[int]=mapped_column(ForeignKey("client_companies.id")); project_role: Mapped[str]=mapped_column(String(100)); notes: Mapped[str|None]=mapped_column(Text)
    project: Mapped[Project]=relationship(back_populates="company_links"); company: Mapped[ClientCompany]=relationship(back_populates="project_links")
class StagedRecord(Base, AuditMixin):
    __tablename__="staged_records"; id: Mapped[int]=mapped_column(primary_key=True); entity_type: Mapped[str]=mapped_column(String(50)); payload_json: Mapped[str]=mapped_column(Text); status: Mapped[VerificationStatus]=mapped_column(Enum(VerificationStatus), default=VerificationStatus.PENDING); source_type: Mapped[str]=mapped_column(String(50)); source_reference: Mapped[str|None]=mapped_column(String(500)); imported_at: Mapped[datetime]=mapped_column(DateTime(timezone=True), default=now); duplicate_contact_id: Mapped[int|None]=mapped_column(ForeignKey("contacts.id")); duplicate_company_id: Mapped[int|None]=mapped_column(ForeignKey("client_companies.id")); duplicate_reasons: Mapped[str|None]=mapped_column(Text); verified_by: Mapped[str|None]=mapped_column(String(255)); verified_at: Mapped[datetime|None]=mapped_column(DateTime(timezone=True)); review_notes: Mapped[str|None]=mapped_column(Text)
    duplicate_contact: Mapped[Contact|None]=relationship(foreign_keys=[duplicate_contact_id]); duplicate_company: Mapped[ClientCompany|None]=relationship(foreign_keys=[duplicate_company_id])
